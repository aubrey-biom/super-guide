...
## `bm_target_schedule_export.txt` and `bm_master_target_raw.md`

The "Brick & Mortar Master Forecast" Google Drive export, "Target Schedule" tab:
the raw slice of the export string (one line, Google-Sheets CSV with row breaks
replaced by spaces) and the same tab re-rowed as a 357 x 51 markdown table for
human reading. Used by `tests/test_bm_master_forecast.py`.
