...
from shipcast.config import iter_numeric_nodes, threshold
...
def test_headline_values(config: dict[str, Any]) -> None:
    ...
    assert threshold(config, "freshness", "rdz_stale_days") == 7
    ...
    # flushables (launch 2026-10-11) order on Monday like personal care: measured on the PO plan
