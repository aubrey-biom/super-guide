> ⚠️ **HISTORICAL DESIGN RECORD — dated 2026-09-03, NOT current.** Read
> `config/target.yaml`, the module docstrings and `scratchpad/` for what the code does;
> where this file disagrees, **the code wins**. In particular everything below about the
> **owner forecast**, the **RDZ Inventory Summary**, the **Inbound Freight Tracker**, the
> **Drive MCP / service-account exports** and the **POS blend** describes a design that
> was REMOVED on 2026-09-08. The monthly POS forecast is now
> `model.consumption.dist_velocity` (one estimator, no blend), `planned_launch` comes from
> Target's own PO plan plus the live `wkly_tcin_item` item state, curated human
> assumptions live in `biom_admin.seed_target_launch_velocity`, and **no code path reads
> Google Drive or any local input file**. This file is kept unedited because it is the
> record of what was designed and why, and rewriting it would destroy that. See
> `scratchpad/inhouse_velocity_forecast_design.md` and
