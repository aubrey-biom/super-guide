...
def repo_root() -> Path:
    """Root holding `config/`, `data/`, `tests/`, `inputs/` and `runs/`.

    `$SHIPCAST_ROOT` if set, else **this package's own directory**
    (`pipelines/target_shipment_forecast`). It was two levels above `src/shipcast`
    upstream, where the package sat in its own repo; in biom_sql the package is the
    root of its own tree so its config and data travel with it rather than landing in
    the repository root next to `pipelines/` and `ddl/`.
    """
    env = os.environ.get("SHIPCAST_ROOT")
    return Path(env).expanduser() if env else _HERE.parent
